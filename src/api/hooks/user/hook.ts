...
export const useProfileUpdate = () => {
  ...
  return useMutation<
    TUserModel['getUserAdminResponseById'],
    Error,
    TUserModel['IUserUpdatePayload']
  >({
    mutationKey: [UserQueryEnum.UserUpdate],
    mutationFn: async (payload: TUserModel['IUserUpdatePayload']) => {
      const res = await axiosInstance.put<TUserModel['getUserAdminResponseById']>(
        `${endpoints.user.users.update}/${payload.id}`,
        payload,
      );
...
export const useProfileUpdateWithImage = () => {
  const queryClient = useQueryClient();

  return useMutation<
    TUserModel['getUserAdminResponseById'],
    Error,
    TUserModel['IProfileUpdatePayload']
  >({
    mutationKey: [UserQueryEnum.UserUpdate],
    mutationFn: async (payload: TUserModel['IProfileUpdatePayload']) => {
      const res = await axiosInstance.patch<TUserModel['getUserAdminResponseById']>(
        endpoints.user.users.updateProfile,
        payload,
        {
          headers: {
